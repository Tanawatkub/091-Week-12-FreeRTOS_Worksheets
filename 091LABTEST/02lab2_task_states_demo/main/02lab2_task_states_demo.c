...
void state_demo_task(void *pvParameters) {
    ...
    while (1) {
        ...
        eTaskState current_state = eRunning;
        count_state_change(prev_state, current_state);
        prev_state = current_state;
        update_state_display(current_state);

        for (int i = 0; i < 1000000; i++) volatile int dummy = i * 2;

        ESP_LOGI(TAG, "Task entering Ready state");
        current_state = eReady;
        count_state_change(prev_state, current_state);
        prev_state = current_state;
        update_state_display(current_state);
        taskYIELD();

        ESP_LOGI(TAG, "Task entering Blocked state (waiting semaphore)");
        current_state = eBlocked;
        count_state_change(prev_state, current_state);
        prev_state = current_state;
        update_state_display(current_state);
        ...
        if (xSemaphoreTake(demo_semaphore, pdMS_TO_TICKS(2000)) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "Timeout waiting semaphore");
        }
...
void ready_state_demo_task(void *pvParameters) {
    while (1) {
        ESP_LOGI(TAG, "Ready demo task running");
...
void control_task(void *pvParameters) {
    ...
    while (1) {
        ...
        extern TaskHandle_t external_delete_handle;
        ...
        if (control_cycle % 30 == 0) {
            ...
            ESP_LOGI(TAG, "StateDemo Task = %s", get_state_name(st));
        }
...
// Self delete + External delete tasks
void self_deleting_task(void *pvParameters) {
    ...
    ESP_LOGI(TAG, "Self-deleting now");
    vTaskDelete(NULL);
}
